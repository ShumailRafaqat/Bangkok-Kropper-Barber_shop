...
export function SpecialOffers() {
  ...
  const [audience, setAudience] = useState<OfferAudience>(() => {
    ...
    return requestedAudience === "women" ? "women" : "men";
  });
            ...
            <article key={offer.id} style={{ animationDelay: `${index * 75}ms` }} className="deals-card group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-background/80 p-5 shadow-[0_20px_50px_-35px_color-mix(in_oklab,var(--color-primary)_45%,transparent)] transition-all duration-300 hover:-translate-y-1 hover:border-primary/70">
              ...
              <div className="mt-6 space-y-3 border-y border-border py-4">
                {offer.items.map((item) => (
                  <div key={item.serviceId} className="flex items-start justify-between gap-4 text-sm">
                    ...
                    <AnniversaryPrice price={item.price} className="shrink-0 text-xs" />
                  </div>
                ))}
              </div>
              ...
              <div className="mt-5 flex items-end justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold uppercase tracking-[0.16em] text-primary">Anniversary 50% OFF: -{anniversaryPrice(offer.originalTotal).toLocaleString()} THB</p>
                  <p className="mt-1 font-display text-3xl text-primary">{anniversaryPrice(offer.originalTotal).toLocaleString()} <span className="text-sm text-muted-foreground">THB</span></p>
                </div>
              ...
              <Link to="/book" search={{ deal: offer.id }} className="mt-6 inline-flex items-center justify-between rounded-full bg-primary px-4 py-3 text-xs font-bold uppercase tracking-[0.18em] text-primary-foreground transition-all hover:bg-accent">

    ...
    return (
      <main className="min-h-screen bg-background px-5 pb-24 pt-32 text-foreground">
        ...
        <div className="mx-auto max-w-3xl">
          <p className="text-xs font-bold uppercase tracking-[0.35em] text-accent">404 · {t.services}</p>
          <h1 className="mt-4 font-display text-3xl uppercase">
            {language === "th" ? "ไม่มีบริการนี้ในเมนู" : "That service is not on the menu."}
          </h1>
          <p className="mt-4 max-w-xl text-muted-foreground">
            {language === "th"
              ? "ดูเมนูบริการทั้งหมดของ Bangkok Kropper หรือพูดคุยกับทีมงานเกี่ยวกับบริการที่คุณต้องการ"
          ...
          >
            {language === "th" ? "ดูบริการทั้งหมด" : "View all services"} <ArrowRight className="size-4" />
          </Link>
        </div>
      </main>
    );
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-32">
        ...
        <section className="mt-8 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-stretch">
          <div className="relative min-h-[22rem] overflow-hidden rounded-sm border border-primary/40 bg-card md:min-h-[30rem]">
            <img
              src={service.image}
              alt={`${localizedName} at Bangkok Kropper`}
              className="absolute inset-0 size-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-background via-background/15 to-transparent" />
            <div className="absolute bottom-7 left-7 right-7">
              <p className="text-xs font-bold uppercase tracking-[0.3em] text-primary">
                Bangkok Kropper · Sukhumvit
              </p>
              <h1 className="mt-3 max-w-xl font-display text-3xl uppercase leading-[0.95] text-foreground md:text-5xl">
                {localizedName}
              </h1>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-between border border-border bg-card/60 p-6 md:p-9">
            <div>
              <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.25em] text-accent">
                <Clock3 className="size-4" /> {service.minutes} {t.minutes} {language === "th" ? "บนเก้าอี้" : "chair time"}
              </p>
              <p className="mt-6 text-lg leading-8 text-muted-foreground">{localizedDescription}</p>
              <p className="mt-5 leading-7 text-muted-foreground">
                {language === "th"
                  ? "บริการที่ออกแบบให้เหมาะกับสภาพผมและสไตล์ของคุณ พร้อมคำแนะนำจากช่างมืออาชีพ"
                  : service.details}
              </p>
            </div>
            <div className="mt-8 border-t border-border pt-6">
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-muted-foreground">
                {language === "th" ? "ราคาบริการ" : "Service price"}
              </p>
              <AnniversaryPrice price={service.price} align="left" className="mt-2 font-display text-3xl neon-text" />
              <Link
                to="/book"
              ...
              >
                {language === "th" ? "จองบริการนี้" : "Book this service"} <ArrowRight className="size-4" />
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="mt-12 grid gap-8 border-y border-border py-10 md:grid-cols-[1fr_1fr]">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.3em] text-accent">
              {language === "th" ? "สิ่งที่รวมอยู่" : "What is included"}
            </p>
            <h2 className="mt-3 font-display text-2xl uppercase">
              {language === "th" ? "ใส่ใจเพื่อผลลัพธ์ที่เรียบร้อย" : "Built for a clean finish"}
            </h2>
            <ul className="mt-6 grid gap-3 sm:grid-cols-2">
              {service.includes.map((item) => (
                <li
                  key={item}
                  className="flex items-center gap-3 border border-border px-4 py-3 text-sm text-muted-foreground"
                >
          ...
          <div className="border-l border-primary/40 pl-6 md:pl-8">
            <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.3em] text-accent">
                <MapPin className="size-4" /> {t.findYourChair}
            </p>
            <div className="mt-5 grid gap-6 sm:grid-cols-2">
              {[
                { name: "Khlong Toei · Sukhumvit", address: ADDRESS, map: MAPS_LINK },
                { name: "Phaya Thai · Phahon Yothin", address: ADDRESS_TWO, map: MAPS_LINK_TWO },
              ].map((location) => (
                <div key={location.name} className="border-t border-border pt-4">
                  <h2 className="font-display text-xl uppercase">{location.name}</h2>
                  <p className="mt-3 text-sm leading-6 text-muted-foreground">{location.address}</p>
                  <a
                    href={location.map}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-primary hover:text-foreground"
                  >
                    {t.openMap} <ArrowRight className="size-3.5" />
                  </a>
                </div>
              ))}
            </div>
            <Link
              to="/services"
              className="mt-7 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.16em] text-primary hover:text-foreground"
            >
              {language === "th" ? "ดูบริการเพิ่มเติม" : "Explore more services"} <ArrowRight className="size-4" />
            </Link>
          </div>
        </section>
      </main>

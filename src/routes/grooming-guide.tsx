  ...
  const featuredArticles = [
    {
      ...
      detail: "From choosing the right cut to preparing for your visit, this guide covers the details that make a Bangkok barber experience feel polished and dependable.",
    ...
    },
    {
      ...
      detail: "Learn how to choose a shape that suits your face, how often to maintain it, and what to discuss with your barber before the service begins.",
        ...
        <section className="mt-16">
          ...
          <div className="mt-8 grid gap-5">
            {featuredArticles.map((article) => (
              <article key={article.title} className="group grid overflow-hidden border border-border bg-secondary/10 md:grid-cols-[0.92fr_1.08fr]">
                <div className="relative min-h-[18rem] overflow-hidden border-b border-border bg-secondary/20 md:min-h-[24rem] md:border-b-0 md:border-r">
...
const thaiGroomingPaths = [
  {
    id: "refresh",
    number: "01",
    title: "ฉันต้องการรีเฟรชแบบสะอาดและเรียบร้อย",
    label: "เรียบร้อย",
    description: "คงสไตล์เดิมไว้ ล้างรูปทรงให้คมขึ้น และออกจากร้านด้วยลุคที่พร้อมใช้งานทุกวัน",
    detail: "เริ่มจากทรงคลาสสิก เฟดแบบที่สะอาดหรือการเก็บขอบหนวด ให้เป็นทางเลือกที่ง่ายที่สุดเมื่ออยากดูดีแต่ไม่อยากเปลี่ยนเอกลักษณ์ของตัวเอง",
    action: "อ่านคู่มือทรงผม",
    slug: "mens-haircut-bangkok",
    maintenance: "ทุก 3–4 สัปดาห์",
    climate: "เหมาะกับอากาศชื้น",
  },
  {
    id: "change",
    number: "02",
    title: "ฉันต้องการเปลี่ยนทิศทางให้ต่างลงไป",
    label: "คมขึ้น",
    description: "เปรียบเทียบรูปทรง เฟด และสัดส่วนก่อนตัดสินใจเลือกสไตล์ที่รู้สึกต่างออกไปชัดเจน",
    detail: "นำรูปอ้างอิงมาและให้ช่างปรับให้เข้ากับแนวผม รูปหน้า และกิจวัตรการจัดทรงแบบของคุณ การเปลี่ยนแปลงที่ดียังคงรู้สึกเป็นส่วนตัว",
    action: "เปรียบเทียบสไตล์เฟด",
    slug: "low-fade-vs-mid-fade",
    maintenance: "ทุก 2–3 สัปดาห์",
    climate: "คมและมีโครงสร้าง",
  },
  {
    id: "advice",
    number: "03",
    title: "ฉันต้องการคำแนะนำในการเลือก",
    label: "เฉพาะตัว",
    description: "ไม่แน่ใจว่าจะจองอะไรดี เริ่มจากการคุยกับทีมงานและให้เราเลือกบริการที่เหมาะกับคุณ",
    detail: "คุณไม่จำเป็นต้องรู้ชื่อทางเทคนิคของทรงผม ให้บอกว่าชอบอะไร ไม่ชอบอะไร และมีเวลาให้จัดทรงเท่าไร",
    action: "จองการปรึกษา",
    slug: null,
    maintenance: "ตัดสินใจร่วมกัน",
    climate: "ออกแบบตามคุณ",
  },
] as const;

function ThaiGroomingGuidePage() {
  const [query, setQuery] = useState("");
  const [activePathId, setActivePathId] = useState<(typeof thaiGroomingPaths)[number]["id"]>("refresh");
  const activePath = thaiGroomingPaths.find((path) => path.id === activePathId) ?? thaiGroomingPaths[0];
  const articles = thaiJournalArticles.filter(
    (article) =>
      article[0].toLowerCase().includes(query.toLowerCase()) ||
      article[1].includes(query) ||
      article[2].includes(query),
  );
    ...
    <div className="min-h-screen bg-background font-sans text-foreground">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-28 md:pt-32">
        <section className="relative overflow-hidden border border-border bg-secondary/20 px-5 py-12 md:px-8 md:py-16">
          ...
          <div className="relative max-w-5xl">
            <h1 className="mt-6 max-w-4xl font-display text-4xl leading-[0.95] text-foreground md:text-6xl">
              บล็อก Bangkok Kropper
            </h1>
            <h2 className="mt-4 max-w-3xl font-display text-2xl leading-tight text-primary md:text-3xl">
              เคล็ดลับการตัดผม ดูแลหนวดเครา และการกรูมมิ่งในกรุงเทพฯ
            </h2>
            <p className="mt-5 max-w-3xl text-base leading-8 text-muted-foreground">
              คำแนะนำจากผู้เชี่ยวชาญ ตั้งแต่การเลือกบาร์เบอร์ใกล้คุณ ไปจนถึงการเลือกทรงผมและสไตล์กรูมมิ่งที่เหมาะกับคุณ
            </p>
            ...
            {[
              {
                ...
                copy: "Learn how people search for top barbers in Bangkok and what makes a reliable grooming experience worth booking.",
                thaiCopy: "เรียนรู้วิธีเลือกบาร์เบอร์ชั้นนำในกรุงเทพฯ และสิ่งที่ทำให้ประสบการณ์กรูมมิ่งคุ้มค่ากับการจองคิว",
                detail:
                ...
                thaiDetail: "ตั้งแต่การเลือกทรงผมที่เหมาะกับคุณไปจนถึงการเตรียมตัวก่อนมาใช้บริการ คู่มือนี้รวมรายละเอียดที่ทำให้ประสบการณ์กับบาร์เบอร์กรุงเทพฯ เป็นมืออาชีพและไว้วางใจได้",
                slug: "best-barber-bangkok",
                image: barberWorkImage,
              },
              {
                ...
                copy: "Understand the difference between beard trim, line-up, styling and professional grooming recommendations.",
                thaiCopy: "ทำความเข้าใจความแตกต่างระหว่างการเล็มหนวด การเก็บขอบ การจัดแต่งทรง และคำแนะนำจากช่างมืออาชีพ",
                detail:
                ...
                thaiDetail: "เรียนรู้วิธีเลือกรูปทรงให้เข้ากับใบหน้า ความถี่ในการดูแล และสิ่งที่ควรพูดคุยกับช่างก่อนเริ่มบริการ",
                slug: "beard-grooming-bangkok",
                image: "/A Close Shave.png",
              },
            ].map((article) => (
              <article
                key={article.title}
                className="group grid overflow-hidden border border-border bg-secondary/10 md:grid-cols-[0.92fr_1.08fr]"
              >
                <div className="relative min-h-[18rem] overflow-hidden border-b border-border bg-secondary/20 md:min-h-[24rem] md:border-b-0 md:border-r">
                  <img
                    src={article.image}
                ...
                <div className="flex flex-col justify-between p-6 md:p-10">
                  ...
                  <div className="mt-5 space-y-4 text-sm leading-7 text-muted-foreground">
                    <p>{article.thaiCopy}</p>
                    <p>{article.thaiDetail}</p>
                  </div>
                  <Link
                    to="/barber-journal/$articleSlug"
                    params={{ articleSlug: article.slug }}
                    className="mt-8 inline-flex self-center items-center gap-2 rounded-sm bg-primary px-5 py-3 text-xs font-bold uppercase tracking-[0.18em] text-primary-foreground transition-colors hover:bg-accent"
                  >
                    อ่านบทความ <ArrowRight className="size-3.5" />
                  </Link>
                </div>
              </article>
            ))}
        ...
        <section className="relative mt-20 overflow-hidden border border-border bg-secondary/10 p-6 md:p-10">
          ...
          <div className="relative grid gap-10 lg:grid-cols-[0.75fr_1.25fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-4 max-w-lg font-display text-2xl leading-tight text-foreground md:text-4xl">
                ค้นหาลุคถัดไปของคุณ
              </h2>
              <p className="mt-5 max-w-md text-xs leading-6 text-muted-foreground md:text-sm">
                ลืมชื่อทางเทคนิคไปก่อน แล้วเลือกอารมณ์ที่ต้องการ จากนั้นเปิดอ่านคำแนะนำที่เข้ากับผมและกิจวัตรของคุณ
              </p>

              <div className="mt-8 space-y-2">
                {thaiGroomingPaths.map((path) => {
                  const isActive = activePath.id === path.id;
                  return (
                    <button
                      key={path.id}
                      ...
                      className={`flex w-full items-center gap-4 border p-4 text-left transition-colors ${isActive ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background/40 text-foreground hover:border-primary"}`}
                    >
                      <span className={`font-display text-lg ${isActive ? "text-primary-foreground" : "text-primary"}`}>
                        {path.number}
                      </span>
                      <span className="flex-1 text-xs font-semibold md:text-sm">{path.label}</span>
                      <ArrowRight className={`size-4 ${isActive ? "translate-x-1" : "text-primary"}`} />
            ...
            <div className="border border-border bg-background/80 p-6 md:p-8">
              <div className="flex items-start justify-between gap-5 border-b border-border pb-6">
                <div>
                  <p className="text-[0.65rem] font-bold uppercase tracking-[0.25em] text-primary">
                    ทิศทางของคุณ
                  </p>
              ...
              {activePath.slug ? (
                <Link
                  to="/barber-journal/$articleSlug"
                  params={{ articleSlug: activePath.slug }}
                  className="mt-8 inline-flex items-center gap-2 bg-primary px-5 py-3 text-[0.65rem] font-bold uppercase tracking-[0.16em] text-primary-foreground hover:bg-accent"
                >
                  {activePath.action} <ArrowRight className="size-4" />
                </Link>
              ) : (
        ...
        <section className="mt-20">
          <div className="flex items-end justify-between gap-4 border-b border-border pb-5">
            <div>
              ...
              <h2 className="mt-3 font-display text-3xl text-foreground md:text-4xl">
                บทความที่อัปเดตจากประเด็นกรูมมิ่งในกรุงเทพฯ
              </h2>
            </div>
            <span className="text-sm text-muted-foreground">{articles.length} บทความ</span>
          </div>

          <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {articles.map(([title, category, excerpt, slug]) => (
              <article key={slug} className="flex h-full flex-col overflow-hidden border border-border bg-secondary/10 transition-transform hover:-translate-y-1">
                <div className="border-b border-border bg-[radial-gradient(circle_at_top,_rgba(205,152,87,0.12),_transparent_55%)] p-4">
                ...
                <div className="flex flex-1 flex-col p-5">
                  ...
                  <p className="mt-4 flex-1 text-sm leading-7 text-muted-foreground">{excerpt}</p>
                  <div className="mt-6 flex items-center justify-between border-t border-border pt-4">
                    <Link
                      to="/barber-journal/$articleSlug"
        ...
        <section className="mt-20 border-t border-border pt-12 text-center">
          <p className="text-[0.62rem] font-bold uppercase tracking-[0.25em] text-primary">
            ต้องการคำแนะนำหรือไม่
          </p>
          <h2 className="mt-4 font-display text-3xl text-foreground md:text-5xl">
            เลือกบริการที่เหมาะกับสไตล์ของคุณ
          </h2>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link
              to="/book"

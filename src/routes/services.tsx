    ...
    <div className="min-h-screen bg-background font-sans">
      ...
      <main className="mx-auto max-w-7xl px-5 pb-24 pt-32">
        <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.4em] text-accent">
          <span className="size-2 rounded-full bg-primary" /> {t.servicesIntroLabel}
        </p>
        <h1 className="mt-5 max-w-4xl font-display text-3xl leading-[0.98] text-foreground md:text-5xl">
          {t.servicesHeadline}
        </h1>
        <p className="mt-6 max-w-3xl text-base leading-7 text-muted-foreground md:text-lg">
          {t.servicesDescription}
        </p>
        <div id="service-menu" className="mt-12">
          <ServicesExplorer />
        </div>
        <section className="mt-16 border-t border-border pt-10" aria-labelledby="popular-services">
          ...
          <h2
            id="popular-services"
            className="mt-3 font-display text-2xl uppercase text-foreground"
          >
            {t.findExactService}
          </h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {([
              [language === "th" ? "ตัดผมผู้ชาย" : "Men's Haircut", "men"],
              [language === "th" ? "ตัดผมผู้หญิง" : "Women's Haircut", "women"],
              [language === "th" ? "ตัดผมเด็ก" : "Kids' Haircut", "kids"],
            ] as const).map(([label, styleSlug]) => (
              <Link
                key={styleSlug}
                to="/styles/$styleSlug"
                params={{ styleSlug }}
                className="border border-primary/60 bg-primary/5 px-4 py-4 text-sm font-bold text-foreground transition-colors hover:bg-primary hover:text-primary-foreground"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            {seoServices.filter((service) => !["men's haircut", "women's haircut", "kids' haircut"].includes(service.name.toLowerCase())).map((service) => (
              <Link
                key={service.slug}
                to="/$serviceSlug"

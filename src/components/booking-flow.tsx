...
const bookableServices: Svc[] = [
  ...
  },
  {
    ...
    price: 350,
    minutes: 45,
    desc: "Clean, cared-for hands with a precise finish.",
    Icon: Sparkles,
  },
  {
    id: "pedicure-feet",
...
const COUNTRY_PHONE_LIMITS: Record<string, number> = {
  "+1": 10, "+7": 10, "+20": 9, "+27": 9, "+30": 10, "+31": 9, "+32": 9, "+33": 9, "+34": 9, "+36": 9,
  "+39": 9, "+41": 9, "+43": 10, "+44": 10, "+45": 8, "+46": 9, "+47": 8, "+48": 9, "+49": 11, "+51": 9,
  "+52": 10, "+54": 10, "+55": 11, "+56": 9, "+57": 10, "+60": 9, "+61": 9, "+62": 11, "+63": 10, "+64": 9,
  "+65": 8, "+66": 9, "+81": 10, "+82": 10, "+84": 9, "+86": 11, "+90": 10, "+91": 10, "+92": 10, "+94": 9,
  "+95": 9, "+98": 10, "+211": 9, "+212": 9, "+213": 9, "+216": 8, "+218": 9, "+220": 7, "+221": 9, "+223": 8,
  "+224": 8, "+225": 8, "+226": 8, "+227": 8, "+228": 8, "+229": 8, "+230": 8, "+231": 7, "+232": 8, "+233": 9,
  "+234": 10, "+236": 8, "+237": 9, "+238": 8, "+245": 7, "+250": 9, "+251": 9, "+254": 9, "+255": 9, "+256": 9,
  "+257": 8, "+260": 9, "+261": 9, "+263": 9, "+264": 9, "+265": 8, "+266": 8, "+267": 8, "+268": 7, "+291": 7,
  "+351": 9, "+352": 6, "+353": 9, "+354": 7, "+355": 9, "+356": 8, "+357": 8, "+358": 11, "+370": 8, "+371": 8,
  "+372": 7, "+373": 8, "+374": 8, "+375": 9, "+376": 6, "+380": 9, "+381": 9, "+385": 9, "+386": 8, "+420": 9,
  "+421": 9, "+500": 5, "+501": 7, "+502": 8, "+503": 8, "+504": 8, "+505": 8, "+506": 8, "+507": 8, "+509": 8,
  "+591": 8, "+593": 9, "+595": 9, "+598": 8, "+670": 7, "+675": 8, "+676": 7, "+677": 5, "+678": 5, "+679": 7,
  "+680": 7, "+685": 7, "+686": 8, "+687": 6, "+688": 5, "+689": 6, "+691": 7, "+692": 7, "+850": 9, "+852": 8,
  "+853": 8, "+886": 9, "+960": 7, "+961": 8, "+962": 9, "+963": 9, "+964": 10, "+965": 8, "+966": 9, "+967": 9,
  "+968": 8, "+971": 9, "+972": 9, "+973": 8, "+974": 8, "+975": 8, "+976": 8, "+977": 10, "+992": 9, "+993": 8,
  "+994": 9, "+995": 9, "+996": 9, "+998": 9, "+1242": 10, "+1246": 10, "+1264": 7, "+1284": 7, "+1345": 10,
  "+1441": 10, "+1473": 10, "+1649": 10, "+1664": 10, "+1670": 10, "+1671": 10, "+1684": 10, "+1758": 7,
  "+1767": 7, "+1784": 7, "+1787": 10, "+1868": 10, "+1869": 10, "+1876": 10,
};
...
function getPhonePlaceholder(code: string) {
  const groups = COUNTRY_PHONE_GROUPS[code];
  if (groups) return groups.map((size) => "x".repeat(size)).join(" ");
...
function CountryCodePicker({ value, onChange }: CountryCodePickerProps) {
  ...
  const selected =
    SORTED_COUNTRY_CODES.find((country) => country.code === value) ?? SORTED_COUNTRY_CODES[0]!;
  const filteredCountries = SORTED_COUNTRY_CODES.filter((country) =>
    `${country.label} ${country.code}`.toLowerCase().includes(query.trim().toLowerCase()),
  );
  ...
  return (
    <div className="relative w-[6.5rem] shrink-0">
      <button
        ...
        className="flex w-full items-center gap-1 rounded-sm border border-border bg-background px-2 py-2 text-sm font-medium text-foreground"
        aria-expanded={open}
      ...
      {open && (
        <div className="absolute left-0 top-full z-30 mt-1 w-72 overflow-hidden rounded-sm border border-primary/50 bg-background shadow-2xl">
          <input
            autoFocus
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Search country..."
            className="w-full border-b border-border bg-background px-3 py-3 text-sm text-foreground outline-none placeholder:text-muted-foreground"
            aria-label="Search country"
          />
          <div className="max-h-60 overflow-y-auto p-1" role="listbox" aria-label="Countries">
            {filteredCountries.length > 0 ? (
              filteredCountries.map((country) => (
                <button
                  ...
                  className={`flex w-full items-center gap-2 rounded-sm px-2 py-2 text-left text-sm hover:bg-primary/15 ${country.code === value ? "bg-primary/10 text-primary" : "text-foreground"}`}
                  role="option"
              ...
              <p className="px-3 py-4 text-sm text-muted-foreground">No country found.</p>
            )}
          </div>
        </div>
      )}
...
function buildLineUrl(message: string) {
  const encodedId = encodeURIComponent(LINE_ACCOUNT_ID);
...
type BookingFlowProps = {
  ...
  anniversaryPromo?: boolean;
};

export function BookingFlow({
  initialPrompt = "none",
  anniversaryPromo = false,
}: BookingFlowProps) {
  ...
  const [submittedServiceSearch, setSubmittedServiceSearch] = useState("");
  ...
  const canNext =
    ...
    (step === 3 && location && name.trim().length > 1) ||
    step === 4;
    ...
    if (!canNext) {
      setError(
        step === 0
          ...
          : "Add your name and choose a location to continue.",
      );
      return;
    }
    setError(null);
    setStep((s) => (s === 0 ? 3 : Math.min(s + 1, 4)));
  };

  const back = () => {
    setError(null);
    setStep((s) => (s === 4 ? 3 : s === 3 ? 0 : 0));
  };
  ...
  const message = () => {
    ...
    return [
      ...
      `Name: ${name.trim()}`,
      `Location: ${location}`,
      `Services: ${bookingServices.map((item) => item.name).join(", ")}`,
      "",
  ...
  if (done) {
    const readyCopy =
      contactMethod === "line"
        ? "We’ve prepared your appointment in LINE. Please press Send in the LINE chat to complete your booking request — we’ll reply to confirm."
        : t.bookingReadyText;
    ...
    return (
      <div className="animate-rise rounded-sm border border-primary/50 bg-card/70 p-10 text-center neon-ring">
        ...
        <h3 className="mt-6 font-display text-2xl uppercase tracking-wide text-primary neon-text md:text-3xl">
          {t.bookingReady}{" "}
          <Check className="ml-1 inline size-6 align-[-0.2em] text-primary" aria-hidden="true" />
        </h3>
        <p className="mx-auto mt-4 max-w-md text-muted-foreground">{readyCopy}</p>
    ...
    <div className="relative min-w-0 max-w-full overflow-visible rounded-sm border border-border bg-card/70 p-4 pb-24 shadow-2xl shadow-black/20 sm:pb-6 md:p-6">
      {showBookingOptions && !contactMethod && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="booking-channel-title"
        >
          <div className="relative w-full max-w-md border border-primary/45 bg-background p-6 text-center shadow-2xl sm:p-8">
            <button
              type="button"
              onClick={() => void navigate({ to: "/" })}
              aria-label={language === "th" ? "ปิดหน้าต่าง" : "Close booking options"}
              className="absolute right-4 top-4 z-10 grid size-9 place-items-center rounded-full border border-primary/60 bg-background text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
            >
              <X className="size-4" />
            </button>
            ...
            <h2 id="booking-channel-title" className="mt-3 font-display text-2xl text-foreground">
              {language === "th" ? "ต้องการจองคิวผ่านช่องทางใด" : "How would you like to book?"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              {language === "th"
                ? "เลือกช่องทางติดต่อก่อน เราจะเตรียมรายละเอียดนัดหมายให้คุณ"
                : "Choose a contact channel first. Your appointment details will be prepared for you."}
            </p>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-3 text-xs font-bold uppercase tracking-[0.14em] text-primary-foreground"
              >
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-full border border-primary/60 px-4 py-3 text-xs font-bold uppercase tracking-[0.14em] text-primary"
              >
        ...
        >
          <div className="relative w-full max-w-md border border-primary/45 bg-background p-6 text-center shadow-2xl sm:p-8">
            <button
              ...
              aria-label={language === "th" ? "ปิดหน้าต่าง" : "Close location options"}
              className="absolute right-4 top-4 z-10 grid size-9 place-items-center rounded-full border border-primary/60 bg-background text-primary transition-colors hover:bg-primary hover:text-primary-foreground"
            >
              <X className="size-4" />
            </button>
            <MapPin className="mx-auto size-7 text-primary" />
            <h2 id="booking-location-title" className="mt-3 font-display text-2xl text-foreground">
              {language === "th" ? "เลือกสาขาที่ต้องการเข้าใช้บริการ" : "Choose your location"}
            </h2>
            <p className="mt-3 text-sm leading-6 text-muted-foreground">
              {language === "th"
                ? "ต้องการเข้าใช้บริการที่สาขาใด"
                : "Where would you like to visit for your appointment?"}
            </p>
            <div className="mt-6 grid gap-3">
              <button
                type="button"
                onClick={() => {
                  setLocation("Khlong Toei · Sukhumvit — 33 Sukhumvit Rd, Bangkok 10110");
                  setShowLocationOptions(false);
                }}
                className="border border-border px-4 py-3 text-left text-sm text-foreground transition-colors hover:border-primary hover:text-primary"
              >
                <span className="block text-xs font-bold uppercase tracking-[0.18em] text-primary">
                  {language === "th" ? "สาขา 01" : "Location 01"}
                </span>
                Khlong Toei · Sukhumvit
                <span className="mt-1 block text-xs text-muted-foreground">
                  33 Sukhumvit Rd, Bangkok 10110
                </span>
              </button>
              <button
                type="button"
                onClick={() => {
                  setLocation("Phaya Thai · Phahon Yothin — 2/3 Phahon Yothin 7, Bangkok 10400");
                  setShowLocationOptions(false);
                }}
                className="border border-border px-4 py-3 text-left text-sm text-foreground transition-colors hover:border-primary hover:text-primary"
              >
                <span className="block text-xs font-bold uppercase tracking-[0.18em] text-primary">
                  {language === "th" ? "สาขา 02" : "Location 02"}
                </span>
                Phaya Thai · Phahon Yothin
                <span className="mt-1 block text-xs text-muted-foreground">
                  2/3 Phahon Yothin 7, Bangkok 10400
                </span>
              </button>
            </div>
          </div>
      ...
      {showDealNotice && matchedOffer && typeof document !== "undefined"
        ? createPortal(
            <div className="fixed inset-0 z-[70] bg-foreground/30 backdrop-blur-sm">
              <div
                role="dialog"
                aria-labelledby="deal-match-title"
                className="fixed left-1/2 top-32 z-[80] max-h-[calc(100dvh-9rem)] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 overflow-y-auto border border-primary/60 bg-background p-5 text-center shadow-2xl shadow-black/25 sm:top-1/2 sm:max-h-[calc(100dvh-3rem)] sm:-translate-y-1/2 sm:p-6"
              >
                ...
                <h2
                  id="deal-match-title"
                  className="mt-2 font-display text-2xl uppercase leading-none text-foreground"
                >
                  {t.unlockedDeal}
                </h2>
                <p className="mt-3 text-sm leading-6 text-muted-foreground">
                ...
                <div className="mt-5 border-y border-dashed border-border py-4">
                  ...
                  <p className="mt-1 font-display text-3xl text-primary">
                    {matchedOffer.savings.toLocaleString()} THB
                  </p>
                  <p className="text-xs font-bold uppercase tracking-[0.16em] text-accent">
                    {t.dealPrice}: {matchedOffer.dealPrice.toLocaleString()} THB
                  </p>
                </div>
                <button
                  type="button"
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {selectedServices.length > 1
              ? `${selectedServices.length} ${t.serviceLabel} selected`
              : (service?.name ?? t.selectService)}
          </p>
        </div>
      ...
      <div key={step} className="animate-rise mt-5">
        {step === 0 && (
          <>
            ...
            <form
              className="mt-5 flex max-w-2xl items-center gap-3 rounded-sm border border-primary/35 bg-background/60 px-4 py-2 focus-within:border-primary"
              onSubmit={(event) => {
                event.preventDefault();
                setSubmittedServiceSearch(serviceSearch);
              }}
            >
              <Search className="size-4 shrink-0 text-primary" aria-hidden="true" />
              <span className="sr-only">Search booking services</span>
              <input
                type="search"
                value={serviceSearch}
                onChange={(event) => setServiceSearch(event.target.value)}
                placeholder="Search services, beard, color or spa..."
                className="min-w-0 flex-1 bg-transparent py-1 text-sm text-foreground outline-none placeholder:text-muted-foreground"
              />
              <button
                type="submit"
                className="shrink-0 rounded-sm bg-primary px-4 py-2 text-xs font-bold uppercase tracking-[0.14em] text-primary-foreground transition-colors hover:bg-primary/85 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
              >
                Search
              </button>
            </form>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const nextOffer =
                        specialOffers.find((offer) =>
                          offer.items.every((item) =>
                            nextSelection.some(
                              (selectedItem) => selectedItem.id === item.serviceId,
                            ),
                          ),
                        ) ?? null;
                    ...
                    className={`group relative flex min-w-0 max-w-full items-center gap-4 overflow-hidden rounded-sm border p-4 text-left transition-all duration-300 hover:border-primary/70 ${
                      active
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block text-[0.58rem] font-bold uppercase tracking-[0.18em] text-accent">
                        {localizedCategory(id, language)} {t.serviceLabel}
                      </span>
                      <span className="mt-1 block break-words font-display text-base uppercase tracking-wide text-foreground sm:text-lg">
                        {localizedName}
                      </span>
                      <span className="mt-1 block text-xs text-muted-foreground">
                        {getServiceDescription(language, id, desc)}
                      </span>
                    </span>
            ...
            <div className="mt-5">
              <label className="block">
                ...
                <input
                  ...
                  placeholder="John"
                  className="mt-2 w-full max-w-xl rounded-sm border border-input bg-background px-4 py-3.5 text-base text-foreground outline-none transition-colors focus:border-primary"
                />
              </label>
            </div>
          </>
          ...
          <>
            ...
            <div className="mt-6 overflow-hidden rounded-sm border border-primary/40 bg-background/70 neon-ring">
              ...
              <dl className="divide-y divide-border">
                {(() => {
                  const summaryRows: Array<[typeof Scissors, string, string]> = [
                    ...
                    [
                      isLineChannel ? Send : MessageCircle,
                      isLineChannel ? "LINE" : channelLabel(currentChannel),
                      isLineChannel ? "Ready to send" : phone.trim() || "—",
                    ],
                  ];
                  if (note.trim()) {
                    summaryRows.push([
              ...
              {(() => {
                ...
                const discount = selectedServices.reduce(
                  (sum, item) => sum + Math.round(item.price * 0.5),
                  0,
                );
                const finalTotal = originalTotal - discount;
                return (
                  <div className="border-t border-border px-5 py-4 text-right text-sm">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground line-through">
                      Original Total: {originalTotal.toLocaleString()} THB
                    </p>
                    <p className="mt-2 font-semibold text-primary">
                      Anniversary 50% OFF: -{discount.toLocaleString()} THB
                    </p>
                    <p className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-primary">
                      You Save: {discount.toLocaleString()} THB
                    </p>
                    <p className="mt-1 font-display text-2xl text-foreground">
                      Total: <span className="text-primary">{finalTotal.toLocaleString()} THB</span>
                    </p>
                  </div>
                );
              })()}
            ...
            {!isLineChannel && (
              <>
                ...
                <p className="mt-3 text-center text-xs text-muted-foreground">
                  This prepares your appointment in WhatsApp; press Send to confirm.
                </p>
              </>
            )}
          </>

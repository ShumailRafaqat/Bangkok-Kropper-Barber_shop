...
export function SiteHeader() {
  ...
  useEffect(() => {
    ...
    const sectionIds = ["services", "style-lab", "gallery", "locations", "reviews", "team", "faq", "blog"];
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        setActiveSection(closest?.id ?? "home");
      },
      { threshold: [0.15, 0.35, 0.6], rootMargin: "-15% 0px -40% 0px" },
    );
        ...
        <nav className="ml-auto hidden min-w-0 items-center justify-end gap-4 whitespace-nowrap md:flex lg:gap-4 xl:gap-5">
          {nav.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className={`whitespace-nowrap text-[0.61rem] font-semibold uppercase tracking-[0.12em] transition-colors hover:text-primary xl:text-[0.64rem] ${item.to === "/" ? (pathname === "/" && activeSection === "home" ? "border-b border-primary pb-1 text-primary" : "text-muted-foreground") : pathname.startsWith(item.to) || (item.to === "/team" && pathname === "/" && activeSection === "team") || (item.to === "/services" && pathname === "/" && activeSection === "services") || (item.to === "/#style-lab" && pathname === "/" && activeSection === "style-lab") || (item.to === "/grooming-guide" && pathname === "/" && activeSection === "blog") ? "border-b border-primary pb-1 text-primary" : "text-muted-foreground"}`}
            >
              {item.label === "Home"
                ? t.home
                : item.label === "About"
...
const socials = [
  ...
  {
    label: "YouTube",
    href: "https://youtube.com/@bangkokkropper?si=z2ZF30C4bPnwnQPb",
    Icon: Youtube,
  },
...
/** Bangkok local time with weekday index (0 = Monday), independent of device timezone. */
function bangkokNow() {
  ...
  return {
    ...
    day: weekdays.indexOf(values.weekday ?? "Monday"),
  };
}
...
export function SiteFooter() {
  ...
  // Hydration-safe: start with null, update only on client
  const [now, setNow] = useState<ReturnType<typeof bangkokNow> | null>(null);
  ...
  // Safe defaults while loading (server + first client render)
  const current = now ?? { minutes: 0, seconds: 0, day: 0 };
  const today = hours[current.day] ?? hours[0];
  const isOpen = current.minutes >= openingMinutes || current.minutes < closingMinutes;
  const nowTotalSeconds = current.minutes * 60 + current.seconds;
  const openingSeconds = openingMinutes * 60;
  const closingSeconds = closingMinutes * 60;
  const untilSeconds = isOpen
    ? current.minutes >= openingMinutes
  ...
  const clock = now
    ? `${String(Math.floor(current.minutes / 60)).padStart(2, "0")}:${String(
        current.minutes % 60,
      ).padStart(2, "0")}:${String(current.seconds).padStart(2, "0")}`
    : "--:--:--";
  ...
  return (
    <footer className="relative overflow-hidden border-t border-border bg-card">
        ...
        <div>
          ...
          {cuts > 0 && (
            <p className="mt-2 text-xs uppercase tracking-[0.3em] text-accent">
              {cuts} {t.snipText}
              {cuts > 1 && language === "en" ? "s" : ""} —{" "}
              {language === "th" ? "ปัตตาเลี่ยนอุ่นพร้อมแล้ว" : "the clippers are warm"}
            </p>
          )}
          ...
          >
            ...
            {isOpen ? t.openNow : t.closed} · Bangkok <span className="tabular-nums">{clock}</span>
          </div>
              ...
              {hours.map((h, i) => (
                <li
                  ...
                  className={`flex items-center justify-between rounded-sm px-2 py-1 transition-colors ${
                    i === current.day ? "bg-primary/10 text-primary" : "hover:bg-secondary/60"
                  }`}
                >
              ...
              >
                <span className="flex items-center gap-3">
                  <span className="grid size-10 place-items-center rounded-full bg-white shadow-md">
                    <span className="font-sans text-xl font-black text-background">G</span>
                  </span>
                  <span>
      ...
      <p className="relative border-t border-border px-7 py-5 text-center text-xs uppercase tracking-[0.3em] text-muted-foreground">
        © {new Date().getFullYear()} Bangkok Kropper Co., Ltd. · 4.9★ · 667{" "}
